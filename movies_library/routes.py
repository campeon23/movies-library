...
from movies_library.forms import MovieForm, ExtendedMovieForm, MovieForm, RegisterForm, LoginForm
from movies_library.models import Movie, User
...
def index():
    ...
    user = User(**user_data)
    movies_data = current_app.db.movie.find({"_id": {"$in": user.movies}})
...
def edit_movie(_id: str):
    ...
    return render_template("movies_form.html", movie=movie, form=form)
...
@pages.get("/movies/<string:_id>/rate")
@login_required
def rate_movie(_id):
...
@pages.get("/movies/<string:_id>/watch")
@login_required
